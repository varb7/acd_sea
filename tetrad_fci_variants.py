#!/usr/bin/env python3
"""
PyTetrad FCI variants wrapper (BOSS-FCI, GRaSP-FCI, SP-FCI).
All variants are score-based hybrid algorithms that use a score-based search
followed by FCI final orientation rules to handle latent confounders.

Based on py-tetrad: https://github.com/cmu-phil/py-tetrad
"""
...
class TetradFCIBase:
    """Base class for Tetrad FCI variants with common functionality."""
    ...
    def _get_independence_test(self, tetrad_data, cats, cont):
        ...
        if cats and cont:
            # Mixed data: use Conditional Gaussian test
    ...
    def _get_score(self, tetrad_data, cats, cont):
        ...
        if cats and cont:
            ...
            # Continuous data only: use SEM BIC score
            score = self.score_module.SemBicScore(tetrad_data)
            score.setPenaltyDiscount(self.penalty_discount)
        return score

    def _pag_to_adjacency(self, pag, columns):
        """Convert PAG (Partial Ancestral Graph) to adjacency matrix."""
        n = len(columns)
        ...
        for i, a in enumerate(columns):
            ...
            for j, b in enumerate(columns):
                ...
                # Get endpoints relative to node a
                if e.getNode1() == na:
                    ea, eb = e.getEndpoint1(), e.getEndpoint2()
                else:
                    ea, eb = e.getEndpoint2(), e.getEndpoint1()
                
                # If there's a directed edge from a to b (a -> b)
                if ea == Endpoint.TAIL and eb == Endpoint.ARROW:
                    adj[i, j] = 1
                # Include undirected edges if specified
...
class TetradBossFCI(TetradFCIBase):
    """
    BOSS-FCI: Uses BOSS (Best Order Score Search) in place of FGES 
    for the initial step in the GFCI algorithm.
    
    BOSS is a score-based algorithm that searches over permutations.
    This tends to produce more accurate PAGs than GFCI for latent variable cases.
    ...
    def run(self, data: Union[pd.DataFrame, np.ndarray], 
        ...
        columns : list, optional
            Column names (required if data is numpy array)
        prior : dict, optional
            Prior knowledge in the format expected by build_tetrad_knowledge
            
        Returns
        -------
        np.ndarray
            Adjacency matrix of the learned PAG
        """
...
class TetradGraspFCI(TetradFCIBase):
    """
    ...
    GRaSP relaxes the faithfulness assumption and tends to produce more 
    accurate PAGs than GFCI for latent variable cases.
...
class TetradSpFCI(TetradFCIBase):
    """
    SP-FCI (Sparsest Permutation FCI): Uses SP algorithm in place of FGES 
    for the initial step in the GFCI algorithm.
    
    SP considers all permutations, which is exponential in the number of variables.
    Limited to about 10-11 variables total, but can handle more if knowledge 
    tiers are used (up to 11 variables per tier).
    ...
    Note
    ----
    SP is limited to small datasets due to its exponential complexity.
    Use knowledge tiers to extend functionality to more variables.
    """
    ...
    def run(self, data: Union[pd.DataFrame, np.ndarray], 
        ...
        prior : dict, optional
            Prior knowledge in the format expected by build_tetrad_knowledge.
            Knowledge tiers are especially important for SP-FCI to scale beyond
            10-11 variables.
        ...
        ------
        ValueError
            If data has too many variables without knowledge tiers
        """
        if isinstance(data, np.ndarray):
            if columns is None:
                raise ValueError("Column names must be provided for numpy array input.")
        ...
        if len(columns) > 11 and prior is None:
            print(
                ...
                "SP is limited to ~10-11 variables. Consider using knowledge tiers "
                "or a different algorithm (BOSS-FCI or GRaSP-FCI)."
            )
...
def run_boss_fci(
    ...
    Convenience function for TetradBossFCI.run()
    """
...
def run_grasp_fci(
    ...
    Convenience function for TetradGraspFCI.run()
    """
...
def run_sp_fci(
    ...
    Convenience function for TetradSpFCI.run()
    
    Note: SP-FCI is limited to ~10-11 variables without knowledge tiers.
    """
    sp_fci = TetradSpFCI(
        alpha=alpha,
        depth=depth,
...
if __name__ == "__main__":
    # Example usage
    print("Tetrad FCI Variants")
    print("=" * 50)
    print("\nAvailable algorithms:")
    print("1. BOSS-FCI: Uses BOSS for initial search")
    print("2. GRaSP-FCI: Uses GRaSP for initial search")
    print("3. SP-FCI: Uses Sparsest Permutation (limited to ~11 variables)")
    print("\nAll algorithms:")
    print("- Use score-based search followed by FCI orientation")
    print("- Handle latent confounders")
    print("- Support mixed continuous/discrete data")
    print("- Support prior knowledge via knowledge tiers")
