...
def run_generate(mode: str = "grid", config: str = None):
    ...
    data_gen_dir = Path(__file__).parent / "data_generator"
    ...
    if mode == "grid":
        ...
        result = subprocess.run(cmd, cwd=str(data_gen_dir.parent))
        
        if result.returncode == 0:
            index_file = data_gen_dir.parent / "csuite_grid_datasets" / "index.csv"
...
def run_experiments(index_path: str = None, results_path: str = None, with_prior: bool = True):
    ...
    # Run experiments
    inference_dir = Path(__file__).parent / "inference_pipeline"
    ...
    print(f"\nRunning: {' '.join(cmd)}")
    result = subprocess.run(cmd, cwd=str(inference_dir.parent))
