...
- Correct independence-test constructors (no stray args).
...
class TetradRFCI:
    ...
    def _create_independence_test(self, tetrad_data, cats, cont):
        ...
        Choose test by data type:
          ...
          discrete   -> IndTestChiSquare(DataSet, alpha) (+ optional min count)
          continuous -> IndTestFisherZ(DataSet, alpha)
        """
        if cats and cont:
            return self.search.IndTestConditionalGaussianLrt(tetrad_data, self.alpha)
        elif cats and not cont:
            test = self.search.IndTestChiSquare(tetrad_data, self.alpha)
            # Optional: relax/strengthen small-cell handling:
            # test.setMinCountPerCell(5.0)
            return test
        else:
            return self.search.IndTestFisherZ(tetrad_data, self.alpha)

    def _run_rfci(self, indep_test):
        rfci = self.search.Rfci(indep_test)
        # Always set depth (Tetrad uses -1 for unlimited)
...
def run_rfci(
    ...
    rfci = TetradRFCI(alpha=alpha, depth=depth, count_partial=count_partial)
